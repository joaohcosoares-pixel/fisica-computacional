...
# MODIFICAÇÃO FÍSICA: Limite magnético expandido para induzir topologia
...
def generate_dataset(n_samples: int = 5000, N_bz: int = 60, n_occ: int = 3, seed: int = 42, out: Path = CSV_PATH) -> pd.DataFrame:
    ...
    while len(valid_rows) < n_samples:
        ...
        for i in range(batch_size):
            ...
            # Só aceitamos a amostra se for matematicamente estável (c não for None)
            if c is not None:
...
def train_classifier(csv_path: Path = CSV_PATH, epochs: int = 150, batch_size: int = 256, lr: float = 1e-3, val_frac: float = 0.2):
    ...
    X_tr, X_va = scaler.transform(X_raw[tr_idx]), scaler.transform(X_raw[va_idx])

    tr_loader = DataLoader(ChernDataset(X_tr, y[tr_idx]), batch_size=batch_size, shuffle=True)
    ...
    for ep in range(1, epochs + 1):
        ...
        if ep % 10 == 0 or ep == 1:
            print(f"{ep:>4}  {tr_loss:>8.4f}  {va_loss:>8.4f}  {va_acc:>7.4f}")
    ...
    print(f"\nAcurácia de Validação Final (Melhor Modelo): {va_acc:.4f}")
