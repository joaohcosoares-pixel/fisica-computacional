...
def train_classifier(csv_path: Path = CSV_PATH, epochs: int = 150, batch_size: int = 256, lr: float = 1e-3, val_frac: float = 0.2):
    ...
    for ep in range(1, epochs + 1):
        ...
        # Injeção de auditoria visual no console
        if ep % 10 == 0 or epochs_no_improve >= patience:
            print(f"Época {ep:03d} | Limiar Otimizado: {optimal_threshold:.4f} | Precision: {precision:.4f} | Recall: {recall:.4f} | F1-Score: {f1:.4f}")

        if epochs_no_improve >= patience:
            print(f"\n[Intervenção] Early stopping acionado na época {ep}. Restaurando pesos da época {ep - patience}.")
            break
